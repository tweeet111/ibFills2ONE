...
def import_fills():
    ...
    fills_df['NetCash'] = -(fills_df['Quantity'] * fills_df['Multiplier'] * fills_df['TradePrice']) - fills_df[
        'IBCommission'].where(fills_df['Buy/Sell'] == "BOT")
    ...
    # filter for Option Trades only and UL by filter_list ( European options e.g. Covered Calls throw error in ONE)
    filter_list = ['SPX', 'SPY']
    ...
    df_one = df_one[
        ['TradeDate', 'TradeTime', 'Buy/Sell', 'AssetClass', 'Symbol', 'Quantity', 'TradePrice', 'IBCommission',
         'UnderlyingSymbol', 'NetCash']].replace("BOT", "BUY").replace("SLD",
                                                                       "SELL")  # replace signal words "buy/sell" from ib.fills to import format
    ...
    fills_df['Expiration'] = pd.to_datetime(fills_df['Expiration'], errors='coerce', format="%Y-%m-%d") #+ timedelta(days=1)
